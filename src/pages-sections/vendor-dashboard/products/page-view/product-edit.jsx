...
const { GETINDOORBILLBYID_API } = IndoorBillsEndpoints;
...
import { Editinvoiceipdhandler } from "services/operations/ipdinvoices";
...
export default function IpdForm() {
  ...
  useEffect(() => {
    const fetchBill = async () => {
      const response = await apiConnector("GET", `${GETINDOORBILLBYID_API}?slug=${slug}`);
  ...
  const handleFormSubmit = async (values) => {
    ...
    console.log(ndata)
    await Editinvoiceipdhandler(ndata);
  };
  ...
  const INITIAL_VALUES = {
    ...
    DoaTime: bill?.DoaTime || null,
    DodTime: bill?.DodTime || null,
    WardName: bill?.WardName || "",
    services: bill?.services || [{ serviceName: "", servicePrice: "", serviceQty: "", serviceTotal: "" }],
    TotalBillAmount: bill?.TotalBillAmount || "",
    ConsAmount: bill?.ConsAmount || "",
    NetPayAmount: bill?.NetPayAmount || "",
    PaidAmount: bill?.PaidAmount || "",
    DueAmount: bill?.DueAmount || "",
    status: bill?.status || "",
    AdvanceAmount: bill?.AdvanceAmount || "",
    AdvanceBalAmount: bill?.AdvanceBalAmount || "",
    AdvanceRefundAmount: bill?.AdvanceRefundAmount || "",
    PaymentDetails: bill?.PaymentDetails || [{ PaymentType: "", PaymentAmount: "" }],
  };
  ...
  return (
    <PageWrapper title="Edit IPD Invoice">
 <Card className="p-3">
      <Formik
